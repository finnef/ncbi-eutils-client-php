...
class NCBIClientTest extends UnitTestCase
{
    ...
    {
        $this->search_terms = self::generate_search_terms();
        ...
        $this->file_set = new File_Set_DateSequence(array(
            'directory' => getcwd() . '/fetched_client',
            'suffix' => '.xml',
        ));
    ...
    public function test_client()
    {
        $c = $this->client;
        $result = $c->extract();
        $this->assertTrue( count($result['file_names']) == 3 );
        $this->assertIsA( $result['esearch'], 'NCBI_ESearch' );
        $this->assertIsA( $result['efetch'], 'NCBI_EFetch' );

        $download_count = 0;
        foreach ($result['file_names'] as $file_name) {
            $xml = simplexml_load_file( $file_name );
            $file_count = count(array_keys( $xml->xpath('//PubmedArticle') ));
    ...
    public function test_alternate_db()
    {
        $this->file_set->clear();

        $c = new NCBI_Client(array(
            'email' => 'your@email.address',
            'db'    => 'nlmcatalog',
            'tool'  => 'your-tool-name',
            'search_terms' => $this->search_terms,
        ...
        foreach ($result['file_names'] as $file_name) {
            $xml = simplexml_load_file( $file_name );
            $file_count = count(array_keys( $xml->xpath('//NLMCatalogRecord') ));
            $this->assertTrue( $file_count == $this->records_per_download );
            $download_count += $file_count;
        }
        $this->assertTrue( $download_count == $this->count );
