...
class ClientTest extends \PHPUnit_Framework_TestCase
{
  ...
  {
    $this->count = 15;
    $this->recordsPerDownload = 5;
    $this->searchTerms = self::generateSearchTerms();
    $this->email = getenv('NCBI_USER_EMAIL');
    $this->tool = getenv('NCBI_USER_TOOL');
    ...
    $this->fileSet = new DateSequence(array(
      'directory' => dirname(__FILE__) . '/fixtures/fetched-client',
      'suffix' => '.xml',
    ));
  ...
  function testNew()
  {
    $client = new Client(
      ...
      (
        ...
        'recordType' => 'citation',
        'maxRecords' => $this->count,
        'recordsPerDownload' => $this->recordsPerDownload,
        'fileSet' => $this->fileSet,
      )
    );
    $this->assertInstanceOf('\UmnLib\Core\NcbiEUtilsClient\Client', $client);
    return $client;
  }
  ...
  function testClient($client)
  {
    ...
    $this->assertEquals(3, count($result['filenames']));
    $this->assertInstanceOf('\UmnLib\Core\NcbiEUtilsClient\ESearch', $result['esearch']);
    $this->assertInstanceOf('\UmnLib\Core\NcbiEUtilsClient\EFetch', $result['efetch']);
    ...
    $this->assertEquals($this->count, $downloadCount);

    // Cleanup:
    $this->fileSet->clear();
  }

  function testAlternateDb()
  {
    $this->fileSet->clear();
    ...
    foreach ($result['filenames'] as $filename) {
      $xml = simplexml_load_file($filename);
      $fileCount = count(array_keys($xml->xpath('//NLMCatalogRecord')));
      $this->assertEquals($this->recordsPerDownload, $fileCount);
      $downloadCount += $fileCount;
    }
    $this->assertEquals($this->count, $downloadCount);
